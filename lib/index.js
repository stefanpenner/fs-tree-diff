...
FSTree.prototype.readFileSync = function(relativePath, encoding) {
  ...
  if (entry) {
    // if instead of this.root we asked the entry, we could emulate symlinks on readFileSync
...
FSTree.prototype.writeFileSync = function(relativePath, content, options) {
  ...
  if (position === undefined) {
    this._relativePathToChange[relativePath] = this._changes.push(['create', relativePath, entry]) - 1;
  } else {
    this._changes[position][0] = 'create';
