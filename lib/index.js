...
FSTree.prototype.changes = function() {
  ...
  this._ensureEntriesPopulated();
  if(this.srcTree ) {
    var prevTree = new FSTree.fromEntries(this.prevEntries);
    patches = prevTree.calculatePatch(this);
    this.prevEntries = [];
    this.prevEntries = this.entries.slice(0, this.entries.length);
  } else {
    patches = this._changes;
  }

  return patches.filter(change => {
