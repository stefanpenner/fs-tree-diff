...
FSTree.prototype.changes = function() {
  if(this.srcTree && this._changes.length === 0) {
    this._ensureEntriesPopulated();
    return getChangesFromEntries(this.entries);
  } else {
...
function getChangesFromEntries (entries) {
  return entries.map( entry => {
    let operation;
    let path = entry.basePath + entry.relativePath;
    ...
    if(fs.lstatSync(path).isDirectory()) {
     ...
     return [operation, entry.relativePath, entry];
   });
}
...
FSTree.prototype.reread = function(newRoot) {
  if(newRoot && path.resolve(newRoot) !== newRoot) {
