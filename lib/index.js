...
FSTree.prototype.findByRelativePath = function(relativePath, options) {
  const dereferenceFoundEntry = options && options.dereferenceFoundPath !== undefined ? options.dereferenceFoundPath : true;
  ...
  for (let i = 0; i < this.entries.length; i++) {
    ...
    if (entryRelativePath(entry) === chompPathSep(relativePath)) {
     // let projection = entry._projection;
      if (dereferenceFoundEntry && projection && projection.entry !== ROOT) {
...
FSTree.prototype.unlinkSync = function(relativePath) {
  ...
  let result = this.findByRelativePath(relativePath, { dereferenceFoundPath: false });
...
FSTree.prototype.symlinkSyncFromEntry = function(srcFSTree,  srcRelativePath,  destRelativePath /*, type */) {
  ...
  if (srcRelativePath === "/") {
    ...
    sourceEntry = srcFSTree.findByRelativePath(srcRelativePath).entry;
  }
