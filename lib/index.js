...
FSTree.prototype = {
  ...
  set cwd(value) {
    //TODO : add ensureTrailing slash here, and remove ensureTrailingSlash from getter
...
FSTree.prototype.findByRelativePath = function(relativePath, options) {
  // returnSymlinks is used to determine whether to return the current tree or the symlinked tree.
  // if returnSymlinks is true, return the tree after walking through symlinks else return current tree
  const returnSymlinks = options && options.returnSymlinks;
  const followSymlinks = options && options.followSymlinks !== undefined ? options.followSymlinks : true;

  if(!returnSymlinks && !followSymlinks) {
    throw new Error("Either follow or return symlinks");
  }
  ...
  for (let i = 0; i < this.entries.length; i++) {
    ...
    if (entryRelativePath(entry) === chompPathSep(relativePath)) {
      ...
      let walkThroughProjections = !returnSymlinks && projection && projection.entry !== ROOT;
    ...
    } else if(followSymlinks && projection && relativePath.startsWith(ensureTrailingSlash(entry.relativePath))) {
...
function processChangeWithProjections(tree, change, options, filteredEntries) {
  ...
  if (shouldFollowSymlink(change[1], effectiveCwd)) {
    ...
    if( effectiveCwd.indexOf(ensureTrailingSlash(change[1])) === 0) {
...
function processChangeWithoutProjection(tree, change, options, filteredEntries) {
 ...
 if(matchesFilterStack(change, options)) {
...
function matchesFilterStack(change, options) {
  let currentPath = change[1];
...
FSTree.prototype.readFileSync = function(relativePath, encoding) {
  ...
  // console.log("result.tree.root + '/' +  entry.relativePath  -------")
  // console.log(result.tree.root + '/' +  entry.relativePath);
...
FSTree.prototype.unlinkSync = function(relativePath) {
  ...
  let result = this.findByRelativePath(relativePath, { returnSymlinks: true });
...
FSTree.prototype.symlinkSyncFromEntry = function(srcFSTree,  srcRelativePath,  destRelativePath /*, type */) {
  ...
  if (srcRelativePath === "/" || srcRelativePath === "." || srcRelativePath === "") {
    sourceEntry = ROOT;
     ...
     let projection = srcFSTree.findByRelativePath(srcRelativePath, { returnSymlinks: true , followSymlinks: false});
     if(!projection.entry) {
