...
FSTree.prototype._normalizePath = function(relativePath) {
  return path.normalize(chompPathSep(`${this.cwd}${relativePath}`));
};
...
FSTree.prototype.readdirSync = function(relativePath) {
  ...
  if (normalizedPath === '.') {
    ...
    filter = (e => entryRelativePath(e).startsWith(normalizedPathTrailingSlash));
  }

  return this.filter(filter).map(e => e.relativePath.replace(normalizedPathTrailingSlash, ''));
}
