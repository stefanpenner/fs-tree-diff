...
function ensureTrailingSlash(inputPath) {
  return inputPath[inputPath.length-1] === path.sep ? inputPath : `${inputPath}/`;
}
...
FSTree.prototype.changes = function() {
  if (this.srcTree) {
    this._ensureEntriesPopulated();
    const filteredEntries = this._filterEntries(this.entries);
    ...
    return this._filterEntries(this._changes).map(change => {
