...
FSTree.prototype.findByRelativePath = function(relativePath) {
  ...
  for (var i = 0; i < this.entries.length; i++) {
    ...
    } else if(entry._projection && relativePath.startsWith(entry.relativePath)) {
     return entry._projection.findByRelativePath(relativePath);
    }
  }
...
FSTree.prototype.symlinkSyncFromInput = function(srcFSTree,  srcAbsolutePath,  destRelativePath /*, type */) {
  ...
  if (result.entry) {
    throw new Error(
      `Error: Symlink already exists from '${destPath}' to '${srcAbsolutePath}' `
    );
  }
  ...
  let mode = Entry.DIRECTORY_MODE;

  let entry = new Entry(normalizedPath, 0, Date.now(), mode, null);

  let srcRelativePath = path.relative(srcFSTree.root, srcAbsolutePath)

  let inTree =   srcFSTree.chdir(srcRelativePath);

  let entriestemp = inTree.walkEntries();

  let entries = inTree.walkEntries().map(entry => {

    return new Entry(entry.relativePath.replace(srcRelativePath, destRelativePath), entry.size, entry.mtime, entry.mode, entry.checksum);
  })


  entry._projection = FSTree.fromEntries(entries);
