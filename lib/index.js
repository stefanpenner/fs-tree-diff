...
const DEFAULT_DELEGATE = {
  ...
  mkdirp: function(inputPath, outputPath, relativePath) {
...
FSTree.prototype.mkdirpSync = function(relativePath) {
  ...
  mkdirp.sync(`${this.root}${normalizedPath}`, this);

  this.entries = walkSync.entries(this.root);

  for( let i = 0; i< this.entries.length; i++) {
    this._track('mkdirp', this.entries[i]);
    this._insertAt(result, this.entries[i]);
  }
