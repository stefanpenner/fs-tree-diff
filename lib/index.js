...
FSTree.prototype._normalizePath = function(relativePath) {
  ...
  // relative path= ember-data , cwd = modules/ember-data
  // cwd = modules relativePath = modules/ember-data
  var normalizedPath = ensureTrailingSlash(relativePath).indexOf(this.cwd) > -1  ? path.normalize(`${relativePath}`) : this.cwd.indexOf(relativePath) > -1 ? path.normalize(`${this.cwd}`) : path.normalize(`${this.cwd}${relativePath}`);
...
FSTree.prototype.readdirSync = function(relativePath) {
  ...
  if (normalizedPath !== '') {
    ...
    entries = result.entry._projection && result.entry._projection.entry === ROOT ? result.entry._projection.tree.entries : result.tree.entries;
  } else {
    prefix = '';
  }
...
FSTree.prototype._filterEntries = function(options) {
  ...
  //  const filterMatched = filterMatches(entry.relativePath, this.cwd, this.files, this.include, this.exclude);
    ...
    // cwd should always point to the cwd of the current tree and not from options.

    const filterMatched = filterMatches(entry.relativePath, options.cwd, options.files, options.include, options.exclude);
    ...
    } else if (isDirectory(entry) && entry.relativePath.indexOf(options.cwd) > -1) {
...
FSTree.prototype.changes = function(options) {
  ...
  if(options === undefined) {
    options = {
      cwd: this.cwd, files: this.files, include: this.include, exclude: this.exclude
    }
  }
