...
FSTree.prototype._normalizePath = function(relativePath) {
  return lchompPathStart(chompPathSep(path.normalize(`${this._projection.cwd}${relativePath}`)));
};
...
FSTree.prototype.resolvePath = function(relativePath) {
  ...
  if (!resolvedPath.startsWith(rootSansPathSep)) {
    let err;
    if (this._projection.cwd) {
      err = `Invalid path: '${relativePath}' not within dir '${this._projection.cwd}' of root '${this.root}'`;
    } else {
...
FSTree.prototype.chdir = function(relativePath, options) {
  ...
  if (cwd === this._projection.cwd) { return this; }
...
FSTree.prototype.filter = function(fn, context) {
  return this.entries.filter(e => {
    return entryRelativePath(e).indexOf(this._projection.cwd) === 0;
  }).filter(fn, context);
};
