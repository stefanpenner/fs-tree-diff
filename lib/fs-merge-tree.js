...
FSMergeTree.prototype._mergeRelativePath = function (overwriteOptions, baseDir, possibleIndices) {
  ...
  for (i = 0; i < this.length; i++) {
    ...
    for (j = 0; j < names[i].length; j++) {
      ...
      let entry;
      let source = tree.findByRelativePath(filePath);
      let originalEntry = source.entry;
      let originalTree = source.tree;
      //let relativePath = originalEntry.relativePath.indexOf(filePath) > -1 ? originalEntry.relativePath :  filePath;
      let relativePath = filePath;

        entry = new Entry(relativePath, originalEntry.size, originalEntry.mtime, originalEntry.mode, originalEntry.checksum);
       // entry.basePath = originalEntry.basePath ? originalEntry.basePath : originalTree.root;
        entry.absolutePath = path.join( originalEntry.basePath ? originalEntry.basePath : originalTree.root, originalEntry.relativePath);
      ...
      if (infoHash.isDirectory) {
        if (infoHash.indices.length === 1 && canSymlink ) {
          ...
          infoHash.entry.linkDir = true;
           //infoHash.entry._projection = { tree : this[i], entry : this[i].findByRelativePath(infoHash.entry.relativePath).entry};
           infoHash.entry._projection =  this[i].findByRelativePath(infoHash.entry.relativePath);
